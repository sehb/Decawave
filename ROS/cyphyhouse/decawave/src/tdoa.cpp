...
TDOA::TDOA(vec3d_t init_pos)
{
    S_prev.setZero(STATE_DIM);
    S_curr.setZero(STATE_DIM);
    D.setZero(MAX_NR_ANCHORS);
    J.setZero(MAX_NR_ANCHORS, STATE_DIM);
...
void TDOA::storeTdoaData(uint8_t Ar, uint8_t An, float distanceDiff)
{
...
vec3d_t TDOA::getAncPosition(int anc_num) { return anchorPosition[anc_num]; }
